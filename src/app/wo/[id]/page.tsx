...
export default function TrackingNumberServiceDetail() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const docRef = doc(db, "service_requests", id as string); // Mengganti nama collection menjadi service_requests
  ...
  return (
    <ProtectedRoute allowedRoles={[ROLES.USER, ROLES.STAFF, ROLES.MANAGER, ROLES.OWNER, ROLES.ADMIN]}>
      <div>
      <NavbarSwitcher/>
                ...
                {data.foto_customer && (
                  <Image
                    ...
                    onError={(e) => {
                      ...
                      target.src = "https://cdn-icons-png.flaticon.com/512/149/149071.png"; // fallback image
                    }}
                    unoptimized
                  />
                )}
