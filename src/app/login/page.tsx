...
export default function AuthPage() {
    ...
    try {
      if (mode === "login") {
        ...
        if (loggedUser.emailVerified) {
          ...
          setMessage("⚠️ Email belum diverifikasi. Cek inbox kamu!");
        }
        ...
        setMessage("✅ Email verifikasi telah dikirim. Cek inbox kamu!");

        // Optional: tetap di halaman register sampai verifikasi
      } else if (mode === "forgot") {
  ...
  // Optional: Fungsi untuk refresh status verifikasi email
  const refreshVerificationStatus = async () => {
    if (auth.currentUser) {
      await auth.currentUser.reload();
      if (auth.currentUser.emailVerified) {
        setMessage("✅ Email sudah diverifikasi. Sekarang bisa masuk.");
          ...
          <form onSubmit={handleAuth} className="space-y-5">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 border border-gray-300 rounded-lg text-gray-800 placeholder-gray-500 focus:ring-2 focus:ring-indigo-400 focus:outline-none bg-white/90"
                required
              />
            </div>
            ...
            {error && (
              <p className="text-red-500 text-center text-sm bg-red-50 py-2 rounded-lg">{error}</p>
            )}
            {message && (
              <p className="text-green-600 text-center text-sm bg-green-50 py-2 rounded-lg">{message}</p>
            )}
