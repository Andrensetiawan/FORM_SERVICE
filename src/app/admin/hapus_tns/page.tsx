...
export default function HapusTnsPage() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setData(newData);
      // No need to alert success, the UI updates
    } catch (error) {
      ...
      <main className="min-h-screen bg-gray-50 text-slate-800 p-4 sm:p-8">
        <div className="max-w-7xl mx-auto pt-10">
          <header className="mb-8">
            <h1 className="text-3xl font-bold text-slate-900">Hapus Service Request</h1>
            <p className="text-sm text-slate-600 mt-1">Hapus data service request yang tidak diperlukan dari sistem.</p>
          </header>
            ...
            <div className="overflow-x-auto">
              <table id="service-table" className="w-full">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Tanggal Masuk</th>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">No HP</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Tipe</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Cabang</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap min-w-[120px]">Teknisi</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">Aksi</th>
                  </tr>
                </thead>
                    ...
                    displayData.map((item, index) => {
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                          <td className="px-4 py-3 text-sm text-slate-700">{tanggal}</td>
                          ...
                          <td className="px-4 py-3 text-sm text-slate-700">{item.no_hp || "-"}</td>
                          <td className="px-4 py-3 text-sm text-slate-700 max-w-[150px] whitespace-nowrap overflow-hidden text-ellipsis">{item.tipe || "-"}</td>
                          <td className="px-4 py-3 text-sm text-slate-700 max-w-[150px] whitespace-nowrap overflow-hidden text-ellipsis">{item.cabang || "-"}</td>
                          <td className="px-4 py-3 text-sm text-slate-700 max-w-[150px] whitespace-nowrap overflow-hidden text-ellipsis">{item.assignedTechnician || "-"}</td>
                          <td className="px-4 py-3 text-sm">
                            <button
                              onClick={() => openConfirmationModal(item.id, item.track_number)}
                              className="bg-red-500 hover:bg-red-600 text-white font-bold py-1 px-3 rounded-md text-xs transition-all duration-150 ease-in-out shadow hover:shadow-lg"
                            >
                              Hapus
                            </button>
                          </td>
