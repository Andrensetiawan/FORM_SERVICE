...
const MediaUploadSection = forwardRef(({
  ...
  return (
    <div className="bg-[#0f1c33] p-4 rounded-xl border border-blue-900/30 shadow-lg">
      <h4 className="text-yellow-400 font-semibold mb-3">{title}</h4>
        <div className="flex flex-col items-center">
            ...
            <div className="flex flex-wrap justify-center gap-2">
                {showCamera && <button onClick={capturePhoto} disabled={!cameraActive} className="px-4 py-2 bg-blue-600 rounded text-xs disabled:bg-gray-500 flex items-center gap-2"><Camera size={14}/> Ambil Foto</button>}
                <button onClick={() => fileInputRef.current?.click()} className="px-4 py-2 bg-indigo-600 rounded text-xs flex items-center gap-2"><FileUp size={14} /> Pilih File</button>
                <input type="file" ref={fileInputRef} onChange={handleFileSelect} multiple accept="image/*,video/*" className="hidden" />
                {showCamera && <button onClick={() => (cameraActive ? stopCamera() : startCamera())} className="px-4 py-2 bg-gray-700 rounded text-xs flex items-center gap-2">{cameraActive ? <CameraOff size={14}/> : <Camera size={14}/>} {cameraActive ? "Matikan" : "Nyalakan"}</button>}
            </div>
        </div>
        ...
        {mediaItems.length > 0 && (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                <div key={item.id} className="relative group">
                    ...
                    <button onClick={() => deleteMedia(item.id)} disabled={deletingId === item.id} className="absolute top-1 right-1 p-1 bg-red-600/80 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity disabled:opacity-100"><Trash2 size={12} /></button>
            ...
            {mode === 'standalone' && (
                 <div className="flex justify-center gap-3 mt-3">
                    <button onClick={saveMedia} disabled={uploading || !hasUnsavedMedia} className="px-6 py-2 bg-green-600 text-sm rounded disabled:opacity-50">
