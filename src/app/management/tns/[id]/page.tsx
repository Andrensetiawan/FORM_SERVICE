      ...
      <div className="min-h-screen bg-[#0d1117] text-white">
        ...
        <div className="pt-24 max-w-6xl mx-auto p-6 space-y-6">
          ...
          <section className="grid md:grid-cols-2 gap-4">
            <div className="bg-gray-800/70 border border-gray-700 rounded-xl p-4 space-y-1">
              <h2 className="font-semibold text-lg mb-2">🧍 Data Customer</h2>
              <p>Nama: {serviceData.nama || "-"}</p>
              <p>No HP: {serviceData.no_hp || "-"}</p>
              <p>Email: {serviceData.email || "-"}</p>
              <p>Alamat: {serviceData.alamat || "-"}</p>
              <p>Cabang: {serviceData.cabang || "-"}</p>
              <p>Penerima Service: {serviceData.penerima_service || "-"}</p>
              <p>
                Tanggal Masuk:{" "}
                {formatDateTime(serviceData.timestamp)}
              </p>
            </div>

            <div className="bg-gray-800/70 border border-gray-700 rounded-xl p-4 space-y-1">
              <h2 className="font-semibold text-lg mb-2">💻 Data Perangkat</h2>
              <p>Merk: {serviceData.merk || "-"}</p>
              <p>Tipe: {serviceData.tipe || "-"}</p>
              <p>Serial Number: {serviceData.serial_number || "-"}</p>
              <p>Keluhan: {serviceData.keluhan || "-"}</p>
              <p>Spesifikasi: {serviceData.spesifikasi_teknis || "-"}</p>
              <p>
                Kondisi:{" "}
                {(serviceData.kondisi || []).join(", ") || "-"}
              </p>
              <p>Prioritas: {serviceData.prioritas_service || "-"}</p>
            </div>
          </section>
